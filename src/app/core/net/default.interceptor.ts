...
export class DefaultInterceptor implements HttpInterceptor {
    ...
    /**
     * 请求添加认证信息
     * @param  {HttpRequest} req - REQ
     * @param  {string} authKey - key
     * @returns {req}
     */
    private appendAuth(req, accessToken) {
        let header = null;
        header = req.headers.set('Authorization', 'Bearer ' + accessToken);
        // 统一加上服务端前缀
        let url = req.url;
        if (url.includes('auth/') || url.includes('login.json') || url.includes('assets/')){
            //
